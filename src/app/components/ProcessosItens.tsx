...
useEffect(() => {
  ...
  const handleSequencia =()=>{
    if(vinAtual && vinAtual.length === 3){
      ...
      alert('digite uma sequencia valida')
    }
  ...
  return(
    <div className="flex items-center justify-center flex-col">
      {!viewProcesso && !showOperacao && !showList &&
       <div className="">
         <input className="text-black p-2 m-2 border border-gray-500 "
           placeholder="Digite a sequencia" 
           type="tel"
           value={vinAtual}
           onChange={(e) => {
            const value = e.target.value.replace(/\D/g, ""); // Remove caracteres não numéricos
             setVinAtual(value);
                 ...
                 />
       <button className="bg-blue-700 rounded-md p-1" onClick={handleSequencia}>Add sequencia</button>
       <button className="bg-green-400 rounded-md p-1 " onClick={mostrarLista}>Mostrar Lista</button>
       </div>}
       {viewProcesso && (
  <div className="grid grid-cols-2 gap-4">
    {processos.map((proc, index) => (
      ...
      showOperacao && 
      <div> 
        <button className="bg-green-500 text-white font-bold py-2 px-4 rounded hover:bg-green-600 transition" onClick={()=>addOperacao(processos[processoAtual])}>{processos[processoAtual].operacao}</button>
      </div>
      }
      
     {showList &&  <ListaFacilit listaCarros={listaCarros} vin={vinAtual} onclick={handleVoltar}/>}
     { showList && <button
